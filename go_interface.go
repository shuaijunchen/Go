...
// EmployeeGetter 接口中嵌入了 PeopleGetter 接口，前者将获取后者所有的方法
...
func (self CallbackFunc) Execute() {
	self()
}

func main() {
	// 空接口的使用，空接口类型的变量可以保存任何类型的值
	// 空接口类型的变量非常类似于弱类型语言中的变量
	...
	varElmployee := Employee{
		name:   "jack Ma",
		age:    20,
		salary: 10000000,
		gender: "Male",
	}
	fmt.Println("varElmployee is:", varElmployee)
	varElmployee.Help()
	fmt.Println("varElmployee.name = ", varElmployee.GetName())
	fmt.Println("varElmployee.age = ", varElmployee.GetAge())
	fmt.Println("varElmployee.salary = ", varElmployee.GetSalary())
	...
	var varElmpInter EmployeeGetter = &varElmployee
	switch varElmpInter.(type) {
	case nil:
		fmt.Println("nil")
	case PeopleGetter:
	...
	// 这种做法的优势是函数显示地 "实现" 特定接口
	varCallbacker := CallbackFunc(func() {
		println("I am a callback function.")
	})
